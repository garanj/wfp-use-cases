...
evaluationDependsOn(":wearApp:watchface")
...
androidComponents.onVariants { variant ->
    ...
    val copyTaskProvider = tasks.register<Copy>("copyWatchface${capsVariant}Output") {
        val wfTask = project(":wearApp:watchface").tasks.named("assemble$capsVariant")
        dependsOn(wfTask)
        val buildDir = project(":wearApp:watchface").layout.buildDirectory.asFileTree.matching {
            include("**/*.apk")
        }
        from(buildDir)
